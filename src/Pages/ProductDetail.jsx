...
const products = [
  ...
  },
  {
   ...
   {
    id: 'hydraulic-pit-stack-parking-system-independent',
    name: 'Hydraulic Pit Stack Parking System – Independent',
    tagline: 'Advanced Independent Parking for Premium Spaces',
    about: 'The Hydraulic Pit Stack Parking System (Independent Type) is an advanced parking solution designed to maximize available space without the limitations of conventional stackers. Unlike the dependent type, this system allows vehicles to be parked and retrieved independently, eliminating the need to move one car to access another. It is the perfect choice for premium residential complexes, commercial buildings, office spaces, and high-end villas where convenience and efficiency are key requirements. This system utilizes a pit to create additional parking space without increasing the height of the structure, making the upper platform level with the ground. Vehicles on both levels can be accessed at any time, ensuring a hassle-free parking experience. Powered by a reliable hydraulic mechanism, the system ensures smooth lifting and lowering, low power consumption, and minimal maintenance. Its strong, powder-coated steel structure with advanced anti-corrosion treatment guarantees long-term durability, while built-in safety features such as mechanical locks, anti-fall devices, and overload protection offer complete security. The Independent Pit Stack Parking System is a modern, user-friendly, and future-ready solution to meet the demands of today’s urban parking challenges.',
    image: 'https://placehold.co/1440x960/003366/ffffff?text=Hydraulic+Pit+Stack',
    generalSpecifications: [
      { key: 'System Length', value: '5000 mm' },
      { key: 'System Width', value: '2350 mm – 2850 mm' },
      { key: 'Pallet Width', value: '2000 mm – 2400 mm' },
    ...
    keyBenefits: [
        'Independent Access: Allows vehicles to be parked and retrieved without moving other cars, enhancing convenience.',
        'Space Efficiency: Maximizes parking capacity without increasing structural height, making it ideal for areas with height restrictions.',
        'Smooth Operation: Utilizes a reliable hydraulic mechanism for quiet, efficient lifting and lowering of vehicles.',
        'Durable Construction: Built with strong, powder-coated steel and advanced anti-corrosion treatment for long-term use.',
    ],
  },
    {
    id: 5,
...
const ProductDetailsPage = () => {
    ...
    return (
      <div className="bg-gray-50 min-h-screen flex items-center justify-center px-6">
        <div className="bg-white rounded-2xl shadow-xl p-10 text-center">
          ...
          <button
            onClick={() => navigate(-1)}
            className="px-6 py-3 bg-[#f05a28] text-white rounded-lg shadow hover:bg-[#d94c1d] transition"
          >
            Go Back
          </button>
        </div>
  ...
  return (
    <div className="bg-gray-50 min-h-screen mt-20 py-16">
      <div className="container mx-auto px-6">
        {/* Back Button */}
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 text-[#001f3f] hover:text-[#f05a28] mb-8 transition font-medium"
        >
          <FaArrowLeftLong /> Back
        </button>

        {/* Product Header Section */}
        <div className="grid md:grid-cols-2 gap-12 bg-white shadow-xl rounded-2xl overflow-hidden border-t-4 border-[#f05a28]">
          {/* Left: Image */}
          <div className="flex items-center justify-center rounded-tl-2xl rounded-tr-2xl md:rounded-tr-none md:rounded-bl-2xl p-6">
            <img
              src={product.image}
              alt={product.name}
              className="rounded-xl border-2 border-[#001f3f] w-full h-full object-cover "
            />
          </div>

          {/* Right: Content */}
          <div className="p-8 flex flex-col justify-center">
            <h1 className="text-4xl font-extrabold text-[#001f3f] mb-4">
              {product.name}
            </h1>
            ...
            {(product.benefits || product.keyBenefits) && (
              <div className="grid grid-cols-2 gap-4 mb-6">
                {(product.benefits || product.keyBenefits).slice(0, 4).map((b, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-2 bg-[#3bb7d2]/10 border-l-4 border-[#3bb7d2] rounded-lg px-3 py-2 text-sm shadow-sm"
                  >
                    <CheckCircle className="w-5 h-5 text-[#3bb7d2]" />
            ...
            <button className="px-6 py-3 bg-gradient-to-r from-[#f05a28] to-[#ff7b54] text-white rounded-lg shadow-lg hover:scale-105 transition w-fit font-semibold">
              Enquire Now
            </button>
            </a>
          </div>
        ...
        {/* Specifications */}
        <div className="mt-16">
          <h2 className="text-3xl font-bold text-[#001f3f] flex items-center gap-2 mb-8">
            <Info className="w-7 h-7 text-[#3bb7d2]" /> General Specifications
          </h2>
          <div className="grid md:grid-cols-2 gap-8">
            {product.generalSpecifications.map((spec, idx) => (
              <div
                key={idx}
                className="bg-white border border-[#3bb7d2]/30 rounded-xl p-6 shadow hover:shadow-md transition"
              >
                <h4 className="text-lg font-semibold text-[#f05a28] mb-2">
                  {spec.key}
                </h4>
        ...
        {(product.benefits || product.keyBenefits) && (
          <div className="mt-16">
            <h2 className="text-3xl font-bold text-[#001f3f] flex items-center gap-2 mb-8">
              <Star className="w-7 h-7 text-[#f0c428]" /> Key Benefits of{" "}
              {product.name}
            </h2>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {(product.benefits || product.keyBenefits).map((benefit, i) => (
                <div
                  key={i}
                  className="bg-gradient-to-r from-[#3bb7d2]/10 to-[#001f3f]/5 rounded-xl p-6 shadow flex items-start gap-3 hover:shadow-lg transition"
                >
                  <CheckCircle className="w-6 h-6 text-[#f05a28] mt-1" />
